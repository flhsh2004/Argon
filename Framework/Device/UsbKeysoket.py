...
class UsbKeysoket(ArgonDevice):
    def __init__(self):
        self.dll = ""
        self.hDev = ""
        AppPosition = os.path.abspath('.')
        dllPosition = AppPosition + "//JCOPTransmit.dll"
        self.dll = ctypes.windll.LoadLibrary(dllPosition)
    def openPort(self,port):
        try:
            str = port.encode("utf8")
            name = c_char_p(str)
            self.hDev = self.dll.CCT_Init(name)
            if self.hDev != 0:
                print("Soket端口打开失败")
                return False
            else:
    ...
    def reset(self):
        try:
            srespond = create_string_buffer(255)
            Returnlenr = create_string_buffer(5)
            rtn = self.dll.CCT_Reset(pointer(Returnlenr), srespond)
            #print(type(rtn))
            rtn = hex(rtn)[2:].upper()
            RtnData = srespond.raw.hex()[0:34].upper()
            if RtnData != "0000000000000000000000000000000000":
                print("ATR:" + RtnData)
                return RtnData,True
            else:
                print("KEY复位失败")
    ...
    def senDisplay(self, APDUdata, *SW,expectData = ""):
        ...
        try:
            ...
            rtn = self.dll.CCT_SendApdu(strlen, comm, lenresp, resp)
            rtn = hex(rtn)[2:].upper()
...
if __name__ == '__main__':
    jts = UsbKeysoket()
    jts.openPort("CCT_SOCKET:10.0.109.18:8801")
    jts.reset()
    jts.begintest()
